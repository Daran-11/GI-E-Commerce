...
const InfoCard = ({ title, children }) => (
  <div className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow">
    {title && <h5 className="font-bold text-[#4EAC14] mb-2">{title}</h5>}
    {children}
  </div>
);
          ...
          <InfoCard>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div className="space-y-4 order-last md:order-first px-2 md:px-0">
                <div>
                  <h6 className="text-[#4EAC14] font-semibold ml-2 md:ml-6">รหัสสินค้า</h6>
                  <p className="ml-4 md:ml-12">{qrData.qrcodeId}</p>
                </div>
                <div>
                  <h6 className="text-[#4EAC14] font-semibold ml-2 md:ml-6">สินค้า</h6>
                  <p className="ml-4 md:ml-12">
                    {qrData.certificate?.type} {qrData.certificate?.variety}
                  </p>
                </div>
                <div>
                  <h6 className="text-[#4EAC14] font-semibold ml-2 md:ml-6">ระดับความหวาน</h6>
                  <p className="ml-4 md:ml-12">14-16° brix</p>
                </div>
                <div>
                  <h6 className="text-[#4EAC14] font-semibold ml-2 md:ml-6">วันที่เก็บเกี่ยว</h6>
                  <p className="ml-4 md:ml-12">
                    {qrData.product?.HarvestedAt ? formatDate(qrData.product.HarvestedAt) : "-"}
                  </p>
                </div>
                <div>
                  <h6 className="text-[#4EAC14] font-semibold ml-2 md:ml-6">ที่ตั้งแปลง</h6>
                  {googleMapsUrl && (
                    <a
                      href={googleMapsUrl}
                      target="_blank"
            ...
            {loading ? (
              ...
              certificates.length > 0 && (
                <div className="mt-6">
                  <h6 className="text-[#4EAC14] font-semibold mb-3 ml-2 md:ml-6">ใบรับรอง</h6>
                    ...
                    {certificates.map((cert, index) => (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">เลขที่: {cert.certNumber}</p>
          ...
          <InfoCard>
            <div className="space-y-4 px-2 md:px-4">
              ...
              <div className="mt-4">
                ...
                <p className="ml-2 md:ml-4">มีความหวานปานกลาง</p>
              </div>
        ...
        <section>
          ...
          <InfoCard>
            <div className="space-y-4 px-2 md:px-4">
              ...
              <p className="ml-2 md:ml-4">
                ปริมาณกรดโดยรวม เฉลี่ยร้อยละ 0.45 ความหวานอยู่ระหว่าง 14-16° brix
              </p>
