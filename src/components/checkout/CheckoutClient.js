    ...
    const fetchDefaultAddress = async () => {
      try {
        const res = await fetch(
          `http://localhost:3000/api/users/${userId}/default-address`,
        );
        if (res.ok) {
    ...
    <div className="top-container">
      ...
      {selectedItems.map((item) => (
        <div key={item.productId}>
          ...
          <p>
            <strong>Price:</strong> {item.productPrice || item.product.Price}
          </p>
          <p>
            <strong>Total:</strong>{" "}
      ...
      <button
        className="w-[200px] h-[50px] font-light rounded-xl text-white bg-[#4EAC14] hover:bg-[#84d154]"
        type="button"
