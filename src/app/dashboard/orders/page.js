...
export default function IncomingOrders() {
  ...
  const fetchOrders = async (userId) => {
    try {
      const res = await fetch(
        `/api/users/${userId}/farmer/orders/?query=${query}`
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `/api/users/${session.user.id}/farmer/orders/delivery/${orderId}`,
        {
          method: "POST",
          headers: {
        ...
        <div className="flex space-x-2 mb-4">
          ...
          {Object.keys(statusOptions).map((status) => (
            <button
              key={status}
              color="primary"
              className={`btn p-1 md:p-2 hover:outline outline-2 outline-[#4eac14] rounded-xl ${
        ...
        <table className="min-w-full h-fit border-separate border-spacing-0 bg-white p-6">
          <thead>
            <tr className="text-xs 2xl:text-base bg-gray-100">
              <th
                scope="col"
                className="w-[50px] px-2 pt-5 pb-3 border-b border-r text-start font-normal rounded-tl-lg"
              >
                รหัส
              </th>
              <th
                scope="col"
                className="w-[150px] px-2 pt-5 pb-3 border-b border-r text-start font-normal"
              >
                สถานะคำสั่งซื้อ
              </th>
              <th
                scope="col"
                className="w-[120px] border-b px-2 pt-5 pb-3 border-r text-start font-normal"
              >
                วันที่สั่งซื้อ
              </th>
              <th
                scope="col"
                className="w-[100px] border-b px-2 pt-5 pb-3 border-r text-start font-normal"
              >
                รวม
              </th>
              <th
                scope="col"
                className="w-[200px] border-b px-2 pt-5 pb-3 border-r text-start font-normal"
              >
                ที่อยู่จัดส่ง
              </th>
              <th
              ...
              <th
                scope="col"
                className="w-[150px] border-b px-2 pt-5 pb-3 border-r text-start font-normal"
              >
                บริการขนส่ง
              </th>
              <th
              ...
              <th
                scope="col"
                className="w-[180px] border-b px-2 pt-5 pb-3 border-r text-start font-normal"
              >
                สถานะการชำระเงิน
              </th>
              <th
              ...
              <th
                scope="col"
                className="w-[320px] border-b px-2 pt-5 pb-3 border-r text-start font-normal"
              >
                แอ็คชั่น
              </th>
              <th
                scope="col"
                className="w-[150px] border-b px-2 pt-5 pb-3 text-start font-normal rounded-tr-lg"
              >
                QR Code
              </th>
            </tr>
          ...
          {filteredOrders.length === 0 ? (
            <tfoot>
              <tr className="bg-gray-100 text-right">
                <td
                  colSpan="12"
                  className="border md:px-4 md:py-2 font-normal text-gray-400 text-left"
                >
                ...
                <tr key={order.id} className="hover:bg-gray-100 text-sm">
                  ...
                  <td className="border-b border-r px-2 md:py-2">
                    <div
                      ...
                      } text-center border-2 border-transparent py-1 rounded-3xl`}
                    >
      ...
      >
        ...
        >
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl" id="delivery-modal-title">
              กรอกข้อมูลขนส่ง
            </h2>
