    ...
    useEffect(() => {
        ...
        if (status === "authenticated") {
          const fetchPurchases = async () => {
            try {
                if (session?.user?.id) {
                    const response = await fetch(`http://localhost:3000/api/users/${session.user.id}/purchases`);
                    if (response.ok) {
        ...
        <ul className="space-y-4">
          {purchases.map((purchase) => (
            <li key={purchase.id} className="border p-4 rounded-lg">
              ...
              <p><strong>Product Name:</strong> {purchase.product.ProductName}</p>
              <p><strong>Quantity:</strong> {purchase.quantity}</p>
              <p><strong>Total Price:</strong> {purchase.totalPrice}</p>
              <p><strong>Address:</strong> {purchase.addressText}</p>
            </li>
          ))}
        </ul>
      )}
